 ...
 * duplication!
 */
#define DRIVER_VERSION "v.1.7.46_android_generic_2"
...
static const signed char qmi_9x15_non_serial_ifaces[]  = {1,4,5,8,10,11,19,20};
...
static int sierra_remove_sysfs_attrs(struct usb_serial_port *port)
{
    ...
    if (port->serial->num_ports > 0) {
		port_tmp = port->serial->port[0];
		if (port_tmp)
			 ...
			 */
			printk("%s usb_get_serial_port_data(port_tmp) = 0x%x\n", __func__, (int)usb_get_serial_port_data(port_tmp));
			kfree(usb_get_serial_port_data(port_tmp));
			printk("kfree at %s\n", __func__);
	}
...
static void sierra_indat_callback(struct urb *urb)
{
	...
	int status = urb->status;
	int last_chunk, count,i;
	#define DEBUG_PRINT_SIZE 50
	...
	else 
	{
		...
		{
			if(port->serial->interface->cur_altsetting->desc.bInterfaceNumber==3)
			{   /*filter out invalid AT port data*/
			...
			/*print complete debug data */
			last_chunk = urb->actual_length%DEBUG_PRINT_SIZE;
			count = urb->actual_length/DEBUG_PRINT_SIZE;
			for(i=0; i < count; i++)
			    usb_serial_debug_data( &port->dev, __func__,DEBUG_PRINT_SIZE, data+i*DEBUG_PRINT_SIZE);
			usb_serial_debug_data( &port->dev, __func__,last_chunk, data+count*DEBUG_PRINT_SIZE);
