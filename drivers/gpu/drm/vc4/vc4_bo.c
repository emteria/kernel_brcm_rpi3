...
void vc4_free_object(struct drm_gem_object *gem_bo)
{
	...
	mutex_lock(&bo->madv_lock);
	if (bo->madv == VC4_MADV_DONTNEED && !atomic_read(&bo->usecnt))
...
int vc4_bo_inc_usecnt(struct vc4_bo *bo)
{
	 ...
	 */
	if (atomic_inc_not_zero(&bo->usecnt))
		return 0;

	mutex_lock(&bo->madv_lock);
	switch (bo->madv) {
	case VC4_MADV_WILLNEED:
		atomic_inc(&bo->usecnt);
		ret = 0;
		break;
...
void vc4_bo_dec_usecnt(struct vc4_bo *bo)
{
	 ...
	 * the madv value.
	 */
	if (atomic_add_unless(&bo->usecnt, -1, 1))
		return;

	mutex_lock(&bo->madv_lock);
	if (atomic_dec_and_test(&bo->usecnt) &&
